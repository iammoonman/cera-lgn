...
function makeMoxfieldDeck(deck_id, player) {
	fetch(`https://api2.moxfield.com/v2/decks/all/${deck_id}`)
		...
		.then((v) => {
			...
			Object.entries(sideboard).forEach(([k, v]) => (v.printingData === undefined ? side_deck.push(...Array(v.quantity).fill({ scryfall_id: v.card.scryfall_id })) : main_deck.push(...Array(v.printingData[0].quantity).fill({ scryfall_id: v.printingData[0].card.scryfall_id }), ...Array(v.printingData[1].quantity).fill({ scryfall_id: v.printingData[1].card.scryfall_id }))));
			...
			main_deck.map(({ scryfall_id }, i) => {
				fetch(`https://api.scryfall.com/cards/${scryfall_id}`)
					.then((r) => r.json())
					.then((r) => {
						const q = world.createObjectFromTemplate("31E5DB224CB620FF0B35E79BB7BB8D02", position.add([0, 0, 0.5]));
						if (["normal", "adventure", "flip", "split", "meld", "leveler", "class", "saga", "planar", "vanguard", "token", "augment", "host"].includes(r.layout)) {
							q.setTextureOverrideURL(r.image_uris.normal.concat(`&scryfall_id=${scryfall_id}&front_face=true`));
						} else {
							q.setTextureOverrideURL(r.card_faces[0].image_uris.normal.concat(`&scryfall_id=${scryfall_id}&front_face=true`));
						}
					});
			});
			side_deck.map(({ scryfall_id }, i) => {
				fetch(`https://api.scryfall.com/cards/${scryfall_id}`)
					.then((r) => r.json())
					.then((r) => {
						const q = world.createObjectFromTemplate("31E5DB224CB620FF0B35E79BB7BB8D02", position.add([0, 8, 0.5]));
						if (["normal", "adventure", "flip", "split", "meld", "leveler", "class", "saga", "planar", "vanguard", "token", "augment", "host"].includes(r.layout)) {
							q.setTextureOverrideURL(r.image_uris.normal.concat(`&scryfall_id=${scryfall_id}&front_face=true`));
						} else {
							q.setTextureOverrideURL(r.card_faces[0].image_uris.normal.concat(`&scryfall_id=${scryfall_id}&front_face=true`));
