...
def get_packs(setcode, num_packs, land_pack=False):
    ...
    for p in all_packs:
        all_cn_sets += p[0]
    set_info = flamewave.collection_import.jsonl_collection(all_cn_sets)
    for p in all_packs:
        new_colle = []
        for crd in p[0]:
...
def get_packs_v3(setcode, num_packs, land_pack=False):
    ...
    for p in all_packs:
        all_cn_sets += p[0]
    set_info = {**flamewave.collection_import.jsonl_collection(all_cn_sets, True), **flamewave.planesculptors.ps_collection(all_cn_sets, True)}
    for p in all_packs:
        new_colle = []
        for crd in p[0]:
...
def get_p1p1_v3(setcode):
    ...
    # Log the seed
    # log["seeds"].append(seed)
    set_info = {**flamewave.collection_import.jsonl_collection(raw_cn_cards, True), **flamewave.planesculptors.ps_collection(raw_cn_cards, True)}
    new_colle = []
