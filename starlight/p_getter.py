...
import flamewave.collection_import
from starlight import number_generator, p_generation
from flamewave import planesculptors
...
def get_packs(setcode, num_packs, land_pack=False):
    ...
    with open(f"setjson/{setcode}.json" if __name__ == "__main__" else f"packcreator/setjson/{setcode}.json", "rb") as f:
        setJSON = json.load(f)
    save = tts_classes.Save(name=f"packs of {setcode}")
    all_packs = []
    for _ in range(num_packs):
        raw_cn_cards, foil_indexes = p_generation.generatepack_c1c2_special(sheet_index_func=lambda a: number_generator.get_number(a), setJSON=setJSON)
...
def get_packs_v3(setcode, num_packs, land_pack=False):
    ...
    with open(f"sj3/{setcode}.json" if __name__ == "__main__" else f"packcreator/sj3/{setcode}.json", "rb") as f:
        setJSON = json.load(f)
    save = tts_classes.Save(name=f"packs of {setJSON['full_name']}")
    # log = {
    #     "seeds": [],
    #     "setcode": setcode,
    #     "num_p": num_packs,
    ...
    if "flag_data" in setJSON.keys():
        if "duplicate_control" in setJSON["flag_data"].keys():
            duplicate_control_list = {k: number_generator.get_sampled_numbers(num_packs * i["per_pack_count"], i["max_sheet_length"]) for k, i in setJSON["flag_data"]["duplicate_control"]["slots_counts"].items()}
            # Log duplicate_control_list
            # log["d_c"] = duplicate_control_list[:]
    all_packs = []
    for _ in range(num_packs):
        raw_cn_cards, foil_indexes, seed = p_generation.pack_gen_v3(set=setJSON, d_c=duplicate_control_list)
        # Log the seed
    ...
    for p in all_packs:
        all_cn_sets += p[0]
    set_info = {**flamewave.collection_import.mm_collection(all_cn_sets, True), **planesculptors.ps_collection(all_cn_sets, True)}
    for p in all_packs:
        new_colle = []
        for crd in p[0]:
...
def get_p1p1_v3(setcode):
    ...
    if "flag_data" in setJSON.keys():
        if "duplicate_control" in setJSON["flag_data"].keys():
            duplicate_control_list = {k: number_generator.get_sampled_numbers(i["per_pack_count"], i["max_sheet_length"]) for k, i in setJSON["flag_data"]["duplicate_control"]["slots_counts"].items()}
            # Log duplicate_control_list
            # log["d_c"] = duplicate_control_list[:]
    raw_cn_cards, foil_indexes, seed = p_generation.pack_gen_v3(set=setJSON, d_c=duplicate_control_list)
    # Log the seed
    # log["seeds"].append(seed)
    set_info = {**flamewave.collection_import.mm_collection(raw_cn_cards, True), **planesculptors.ps_collection(raw_cn_cards, True)}
    new_colle = []
