...
class Flamewave(commands.Cog):
    ...
    async def scry_search(self, ctx: discord.ApplicationContext, query: str):
        ...
        full_set_json = []
        time.sleep(0.25)
        response = requests.get(f"https://api.scryfall.com/cards/search?q={quote(query)}", headers={"User-Agent": "CERA-LGN/0.0"})
        full_json += (resjson := response.json())["data"]
        while resjson["has_more"]:
            time.sleep(0.25)
            response = requests.get(resjson["next_page"], headers={"User-Agent": "CERA-LGN/0.0"})
    ...
    async def set_lands(self, ctx: discord.ApplicationContext, setcode: str):
        await ctx.defer()
        ij_cards = collection_import.jsonl_collection_basics(setcode)
        if len(ij_cards) == 0:
